        ...
        if (v = 0 === v.length ? r.value.match(/[^\/\\]+\..+/g) || [] || "" : v, 0 !== v.length) {
            switch (l.accept) {
                case"file":
                    ...
                    // if (!RegExp("\\w\\.(" + (h || "avi|mp4|wma|rmvb|rm|flash|3gp|flv|mp3|wav|mid|jpg|png|gif|bmp|jpeg") + ")$", "i").test(escape(v))) return o.msg("选择的视频中包含不支持的格式"), r.value = "";
                    if (!RegExp("\\w\\.(" + (h || "bmp|jpg|gif|png|xlsx|xls|csv|doc|docx|ppt|pptx|pdf|txt") + ")$", "i").test(escape(v))) return o.msg("不支持该文件格式"), r.value = "";
                    break;
                case"video":
